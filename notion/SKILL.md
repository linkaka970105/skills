...
For this user, the default database for generic Notion saves is the Media database. See `references/default-media-database.md` for the database ID, URL, properties, and default article mapping.

## Network Troubleshooting

If Notion API requests fail before an HTTP response with TLS EOF/reset or Python `RemoteDisconnected`, do not immediately blame credentials or database sharing. First test routing. See `references/network-routing.md` for the smoke test and the `--noproxy '*' --resolve api.notion.com:443:208.103.161.2` workaround used in this environment.
