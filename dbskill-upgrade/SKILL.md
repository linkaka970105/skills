...
```bash
if [ -d "$HOME/.openclaw/workspace/skills/dbs" ]; then
  INSTALL_DIR="$HOME/.openclaw/workspace/skills"
  echo "Install location: $INSTALL_DIR"
else
  echo "ERROR: dbskill not found in ~/.openclaw/workspace/skills/"
  exit 1
fi
```
...
```bash
OLD_VERSION=$(cat "$HOME/.openclaw/workspace/skills/dbskill-upgrade/../../VERSION" 2>/dev/null || echo "unknown")
...
```bash
BACKUP_DIR="$HOME/.openclaw/workspace/skills/.dbskill-backup-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BACKUP_DIR"
cp -r "$HOME/.openclaw/workspace/skills"/dbs* "$BACKUP_DIR/" 2>/dev/null || true
...
```bash
rm -rf "$HOME/.openclaw/workspace/skills"/dbs*
cp -r "$TMP_DIR/dbskill/skills"/dbs* "$HOME/.openclaw/workspace/skills/"
rm -rf "$TMP_DIR"
...
if [ $? -ne 0 ]; then
  ...
  rm -rf "$HOME/.openclaw/workspace/skills"/dbs*
  cp -r "$BACKUP_DIR"/* "$HOME/.openclaw/workspace/skills/"
...
读取 `$HOME/.openclaw/workspace/skills/dbs/../../README.md`（如果存在），提取从 `OLD_VERSION` 到 `REMOTE_VERSION` 之间的更新内容。
...
- 只支持通过 `~/.openclaw/workspace/skills/` 安装的版本
- 升级前自动备份，失败时自动恢复
- 不需要用户手动操作 git
